...
double generateOutputFile(    // takes an ofstream reference and writes the results
    ...
    const std::vector<std::pair<int, const std::vector<unsigned int>*>>& rangeSearchResults
);

int main(int argc, char *argv[]) {
    string dataset_file;
    string query_file;
    string output_file = "cube_output.txt";
    int k = 10;                           
    int M = 100;
    int Probes = 5;          
    int N = 1;              
    ...
    for (int i=1; i<argc; i++){
        if (strcmp(argv[i], "-d") == 0){
            ...
            if (argc > i +1)
                ...
                cerr << "You did not provide a number after the -M flag" << endl;
                return 1;
            }
            ...
            if (argc > i +1)
                ...
                cerr << "You did not provide a number after the -probes flag" << endl;
                return 1;
            }    
    ...
    while(1){
        ...
        unsigned int testImg = -1;       // Put a random number of an image to print its results (Or -1)
        ...
        for (size_t i=0; i< QUERY_VECS_NUM; i++){ // for every image in the test set
            ...
            auto TrueNeighbours = cube.exhaustiveSearch(*query_vecs[i].second, N);
            ...
            auto rangeNeighbours = cube.approximateRangeSearch(*query_vecs[i].second, R);
...
double generateOutputFile( 
    ...
    for (const auto& approxResult : approximateResults){
        ...
        file << "distanceCube: " << approxDist << endl;
        file << "distanceTrue: " << trueDist << endl;
        trueResult++;
        i++;
    }

    file << "tCube: " << tLSH << endl << "tTrue: " << tTrue << endl;
