...
Clustering::Clustering(const vector<pair<int, vector<unsigned int>*>>& dataset, const vector<pair<int, vector<unsigned int>*>>& dataset_original, const string& configuration_file)
    : vectors(dataset), vectors_original(dataset_original)
{
...
double Clustering::silhouetteCoefficient(size_t i) const {
    ...
    for (size_t idx : VectorsPerCluster[cluster_i]) {
        ...
        // double dist = Functions::distance(*vectors_original[i].second, *vectors_original[idx].second);
...
vector<double> Clustering::averageSilhouette(){
    ...
    */
    ...
    cout << "Average silhouette: " << sum << " / " << vectors.size()<< " = " <<  average << endl;
