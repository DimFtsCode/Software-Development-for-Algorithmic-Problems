...
bool Functions::readInnerEdges(const string& filename, vector<pair<int, vector<int>>>& innerEdges){
    ...
    while (getline(file, line)) {
        ...
        int node;
        char colon; // To skip the colon character in the file

        // Read the node index
        if (!(iss >> node >> colon)) {
            cerr << "Error reading the node index." << endl;
            continue;
        }
        ...
        // Add to innerEdges
        innerEdges.push_back(make_pair(node, neighbors));
    }
